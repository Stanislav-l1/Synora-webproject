...
export default function EditJobPage() {
  ...
  useEffect(() => {
    ...
    api.get<ApiResponse<JobPosting>>(`/career/${id}`)
      .then(res => {
        ...
        if (j.authorId !== user.id) { router.push('/career'); return; }
        setForm({
          title: j.title,
          ...
          experienceYears: j.experienceYears?.toString() || '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await api.put<ApiResponse<JobPosting>>(`/career/${id}`, payload);
  ...
  return (
    <AppShell>
      <div className="max-w-2xl mx-auto px-4 py-6">
        <div className="mb-6">
          <Link href={`/career/${id}`} className="text-sm text-content-secondary hover:text-content-primary">
            ← Back
          </Link>
          <h1 className="text-2xl font-bold text-content-primary mt-2 flex items-center gap-2">
            <Briefcase size={22} className="text-accent" /> Edit Posting
          </h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-surface-secondary border border-border-default rounded-xl p-6 space-y-5">
          ...
          {field('Status',
            <div className="flex gap-2">
              {(['OPEN', 'CLOSED', 'DRAFT'] as JobStatus[]).map(s => (
                <button
                  key={s}
                  type="button"
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="remote" className="text-sm text-content-secondary">Remote / work from anywhere</label>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {field('External application URL',
              <input type="url" value={form.applicationUrl}
          ...
          {field('Skills / stack',
            <div className="space-y-2">
              ...
              {form.skills.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {form.skills.map(s => (
                    <span key={s} className="flex items-center gap-1 px-2.5 py-1 text-xs rounded-full bg-surface-tertiary text-content-secondary border border-border-default">
                      {s}
