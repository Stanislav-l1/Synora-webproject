...
export default function LoginPage() {
  ...
  const { login, isLoading, isAuthenticated, _hydrated, hydrate } = useAuthStore();
  ...
  useEffect(() => {
    if (_hydrated && isAuthenticated) router.replace('/feed');
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const loginVal = (form.get('login') as string).trim();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-cloud px-4 relative overflow-hidden">
      {/* Decorative blobs */}
      <div className="blob-tyrian absolute -top-20 -left-20 w-80 h-80 pointer-events-none" />
      <div className="blob-banana absolute -bottom-20 -right-20 w-96 h-96 pointer-events-none" />
      ...
      <div className="w-full max-w-sm relative">
        ...
        <div className="bg-moss border border-moss-deep rounded-2xl p-8 shadow-xl">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-cloud">{t.auth.loginTitle}</h1>
            <p className="text-sm text-moss-soft mt-1">{t.auth.loginSubtitle}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2 text-moss-soft cursor-pointer">
                <input
                  type="checkbox"
                  className="w-4 h-4 rounded border-moss-velvet bg-moss-deep accent-banana"
                />
                {t.auth.rememberMe}
              </label>
              <Link href="/forgot-password" className="text-banana hover:text-banana-soft transition-colors">
        ...
        <div className="flex items-center justify-between mt-6 text-sm text-cloud-ink/70">
          ...
          <p>
            {t.auth.noAccount}{' '}
            <Link href="/register" className="text-tyrian hover:text-tyrian-soft font-medium transition-colors">
              {t.auth.signUp}
            </Link>
          </p>
        </div>
