    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-96 max-h-[70vh] bg-cloud border border-cloud-deep rounded-lg shadow-xl overflow-hidden flex flex-col z-50">
          ...
          {/* AI Digest card */}
          <div className="px-3 py-2 border-b border-cloud-deep bg-tyrian/5">
            <div className="flex items-start gap-2.5 rounded-lg p-2">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-tyrian/15 text-tyrian mt-0.5">
                <Sparkles size={13} />
              </span>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-cloud-ink leading-tight">{t.ai.digestTitle}</p>
                <p className="text-[11px] text-cloud-muted leading-snug mt-0.5">
                  {t.ai.digestSubtitle}
                </p>
                <ul className="mt-1.5 space-y-0.5">
                  <li className="text-[11px] text-cloud-ink">📊 Your posts got 24 likes this week</li>
                  <li className="text-[11px] text-cloud-ink">🔥 #typescript post is trending now</li>
                  <li className="text-[11px] text-cloud-ink">👥 3 new followers since yesterday</li>
                </ul>
              </div>
              <Link
                href="/assistant"
                onClick={() => setOpen(false)}
                className="text-[10px] text-tyrian hover:text-tyrian-soft font-medium shrink-0 mt-0.5"
              >
                {t.ai.digestBtn}
              </Link>
            </div>
